...
class OpenAIService:
    ...
    DEPRECATED: This service is deprecated in favor of LangGraphService.
    All new functionality should use LangGraphService for better agent capabilities.
    """
    ...
    async def get_nutrition_advice_stream(
        self, user_question: str, user_nutrition_data: Optional[Dict] = None
    ) -> AsyncGenerator[str, None]:
        """Get nutrition advice with streaming response"""

        try:
            prompt = self._get_nutrition_advice_prompt(
                user_question, user_nutrition_data
            )
    ...
    def _get_nutrition_advice_prompt(
        self, user_question: str, user_nutrition_data: Optional[Dict] = None
    ) -> str:
        ...
        if user_nutrition_data:
            ...
            prompt += (
                f"- Калории: {user_nutrition_data.get('total_calories', 0):.1f} ккал\n"
            )
