...
from bot.handlers.photo_analysis import show_nutrition_confirmation
...
"""
    ...
    await callback.message.edit_text(
        text, reply_markup=get_cancel_keyboard(), parse_mode="Markdown"
    )
...
async def process_text_input(
    message: Message, state: FSMContext, food_description: str, portion_info: str
):
    ...
    try:
        ...
        analysis = await nutrition_analyzer.analyze_food_from_text(
...
async def show_text_portion_selection(
...
@router.callback_query(
    TextInputStates.selecting_portion, F.data.startswith("select_portion:")
)
...
@router.callback_query(
    TextInputStates.confirming_nutrition, F.data == "confirm_nutrition"
)
async def confirm_text_nutrition_entry(
...
"""
        ...
        await callback.message.edit_text(
            success_text, reply_markup=get_main_menu_keyboard(), parse_mode="Markdown"
        )
