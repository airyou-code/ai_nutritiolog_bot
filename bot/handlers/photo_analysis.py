...
async def handle_food_photo(
    ...
    try:
        ...
        analysis = await nutrition_analyzer.analyze_food_from_photo(
...
@router.callback_query(
    PhotoAnalysisStates.selecting_portion, F.data.startswith("select_portion:")
)
...
@router.callback_query(
    PhotoAnalysisStates.confirming_nutrition, F.data == "confirm_nutrition"
)
...
@router.callback_query(
    PhotoAnalysisStates.confirming_nutrition, F.data == "change_portion"
)
...
async def show_portion_selection(
    message: Message, analysis: dict, state: FSMContext, edit: bool = False
):
