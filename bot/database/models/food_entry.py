...
from sqlalchemy import BigInteger, DateTime, String, Float, Text, ForeignKey, Date
...
class FoodEntry(Base):
    ...
    portion_size: Mapped[str | None] = mapped_column(String(100), nullable=True)  # e.g., "medium", "large", "small"
    portion_weight: Mapped[float | None] = mapped_column(Float, nullable=True)  # in grams
    ...
    # Metadata
    photo_file_id: Mapped[str | None] = mapped_column(String(255), nullable=True)  # Telegram photo file ID
    input_method: Mapped[str] = mapped_column(String(20), nullable=False)  # "photo" or "text"
    ai_analysis: Mapped[str | None] = mapped_column(Text, nullable=True)  # AI analysis result
    ...
    # Relationships
    user: Mapped["TelegramUser"] = relationship("TelegramUser", back_populates="food_entries")
